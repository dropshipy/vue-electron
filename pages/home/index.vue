<script>
import formatDate from "~/utils/format-date";
import formatCurrency from "~/utils/format-currency";
import copyToClipboard from "~/utils/copy-to-clipboard";

export default {
  computed: {
    userInfo() {
      return this.$store.getters["user/getUserInfo"] || {};
    },
    subscription() {
      return this.$store.getters["subscription/getSubscriptionInfo"];
    },
    dataProfile() {
      return [
        {
          label: "Nama",
          icon: "profile",
          value: this.userInfo.fullName || "-",
        },
        { label: "Email", icon: "email", value: this.userInfo.email || "-" },
        {
          label: "No. HP",
          icon: "phone",
          value: this.userInfo.phoneNumber || "-",
        },
      ];
    },
    isSupportSeller() {
      return this.$config.appName !== "tiksender";
    },
    textStyle() {
      if (this.$config.appName === "tiksender") {
        return "text-primary";
      }
      return "text-[#00AC01]";
    },
  },
  methods: {
    getStatusLabel(status) {
      if (status === "active") return "Aktif";
      if (status === "register") return "Belum aktif";
      if (status === "inactive") return "Tidak aktif";
    },
    dateFormatter(date) {
      return formatDate(date);
    },
    currencyFormatter(amount) {
      return formatCurrency(amount);
    },
    copySubscriptionCode(code) {
      copyToClipboard(code, () => {
        this.$snackbar.success("Berhasil menyalin kode subscription");
      });
    },
  },
};
</script>

<template>
  <Card>
    <div class="flex justify-between flex-wrap gap-1">
      <div class="flex items-center jub gap-2">
        <img
          :src="
            require(`~/assets/icons/profile-circle-filled-${this.$config.appName}.svg`)
          "
          alt="profile"
        />
        <h3 class="text-xl text-dark2 font-bold">Data Profil</h3>
      </div>

      <NuxtLink
        class="flex items-center gap-1 text-blue"
        to="/home/change-password"
      >
        <Icon name="lock" :size="16" />
        <span class="text-sm font-medium">Ubah Password</span>
      </NuxtLink>
    </div>

    <Card primary class="mt-3 w-full max-w-[530px] space-y-2 p-3">
      <div
        v-for="(item, idx) in dataProfile"
        :key="idx"
        class="flex items-center gap-2"
      >
        <Icon :name="item.icon" :class="textStyle" />
        <span class="text-dark2 text-sm"
          >{{ item.label }}: {{ item.value }}</span
        >
      </div>
    </Card>

    <div class="flex items-center gap-2 mt-8">
      <img
        :src="
          require(`~/assets/icons/subscription-filled-${
            !isSupportSeller ? 'red' : 'green'
          }.svg`)
        "
        alt="subscription"
      />
      <h3 class="text-xl text-dark2 font-bold">Data Berlangganan</h3>
    </div>

    <Card
      v-if="!subscription"
      primary
      class="flex flex-col items-center justify-center mt-3 w-full max-w-[530px] p-6"
    >
      <img src="~/assets/icons/empty.svg" alt="empty" />
      <p class="text-sm text-dark2 mt-3">Oops! Kamu belum berlangganan</p>
      <a href="#" :class="textStyle" class="underline text-sm font-medium mt-5"
        >Berlangganan Sekarang</a
      >
    </Card>

    <Card v-else :primary="!isSupportSeller" class="mt-3 w-full">
      <div :class="{ 'border !border-[#60BB55] rounded-xl': isSupportSeller }">
        <div
          class="flex items-center justify-between gap-1 p-3 cursor-pointer rounded-t-[10px]"
          :class="[
            isSupportSeller
              ? 'bg-[#60BB55]/[8%] text-green'
              : 'bg-[#FEB186] text-white',
          ]"
          @click="copySubscriptionCode(subscription.code)"
        >
          <section class="flex items-center gap-1">
            <Icon name="transaction" />
            <p class="font-bold">{{ subscription.code }}</p>
          </section>

          <div
            class="px-5 py-1 bg-opacity-10 border border-opacity-20 text-sm font-medium rounded"
            :class="{
              'bg-green/10 border-green text-green':
                subscription.status === 'active',
              'bg-yellow-500 border-yellow-500 text-yellow-500':
                subscription.status === 'register',
              'bg-red-500 border-red-500 text-red-500':
                subscription.status === 'inactive',
            }"
          >
            {{ getStatusLabel(subscription.status) }}
          </div>
        </div>

        <div class="p-5">
          <div class="flex items-center justify-between">
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                <Icon name="product" :class="textStyle" />
                <p class="font-medium">
                  {{ subscription.subscriptionPlan?.description }}
                </p>
              </div>
              <div class="flex items-center gap-2">
                <Icon name="price-tag" :class="textStyle" />
                <p class="font-medium">
                  {{ currencyFormatter(subscription.subscriptionPlan?.price) }}
                </p>
              </div>
            </div>
          </div>

          <hr class="border-[#FFF0E8] my-3" />

          <div class="space-y-2">
            <div class="flex items-center gap-2">
              <Icon name="date" :class="textStyle" />
              <p>Periode: {{ subscription.cycle }}</p>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-5">
                <Icon
                  v-if="!isSupportSeller"
                  name="symbol-start"
                  class="text-blue"
                />
              </div>
              <p>Tanggal mulai: {{ dateFormatter(subscription.startAt) }}</p>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-5">
                <Icon
                  v-if="!isSupportSeller"
                  name="symbol-start"
                  class="text-primary rotate-180"
                />
              </div>
              <p>Berakhir pada: {{ dateFormatter(subscription.expiredAt) }}</p>
            </div>
          </div>
        </div>
      </div>
    </Card>
  </Card>
</template>
